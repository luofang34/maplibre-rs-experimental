...
Surface paint uses one view zoom for every source level. Terrain quantizes that zoom to one eighth of a level for cache reuse. The bundled palette keeps landcover colour and opacity constant across zooms; feature detail can increase without changing the base palette. A drape is ready only when every source layer is uploaded. A fallback uses complete children or an available ancestor. Coarsened budget targets are requested explicitly and receive upload priority. The texture budget does not coarsen the terrain mesh or its DEM sampling. Host memory reports reduce budgets immediately, while restoring larger budgets requires a recovery margin. A metadata shortage defers the complete texture rather than caching a partial draw. Tile eviction releases both vector and symbol GPU geometry so a reloaded glyph atlas cannot be paired with stale vertices.

An external globe eye at least 60 km above the reference surface uses one vector texture level across the entire view. The level and all ancestor fallbacks must fit the texture budget. A level becomes visible only when every requested texture at that level contains its own uploaded geometry; partial finer arrivals retain the complete coarser level. A newly exposed edge may use an ancestor without downgrading the already visible map. Terrain meshes and DEM sampling keep their independent geometric detail. Near-ground views retain distance-based LOD.

The sky side follows each eye ray’s vertical direction, including when the bookkeeping map center moves behind the eye. Crossing the horizon cannot reverse the sky gradient.
...
Under low memory, vector requests share the remaining slots with DEM requests. Terrain reservation leaves vector work able to progress even when only one slot is free; critical pressure stops both kinds of new work.
